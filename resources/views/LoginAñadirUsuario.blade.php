...
<body class="bg-dark">
    <div class="container mt-5 bg-light p-4 rounded shadow" style="width: 70%;">
        <h2 class="text-center mb-4" style="background-color:green; color:white; padding: 10px;">Registrando nuevo Usuario</h2>
        <form action="" method="">
            @csrf
            <button type="submit" class="btn btn-outline-secondary w-300" style="float: right" formaction="{{route('login')}}">Volver</button>
        </form>
        <hr>
        <form action="{{route('registrousuariologin')}}" method="post" class="bg-light p-4 rounded shadow-sm" >
            @csrf
